...
public class Sample {
    ...
    private static final int TABLE_INTERP_SHIFT = Sample.FP_SHIFT - Sample.LOG2_TABLE_ACCURACY;
    ...
    private static final short[] SINC_TABLE = { 0, 0, 0, 0, 0, 0, 0, 32767, 0, 0, 0, 0, 0, 0, 0, 0, -1, 7, -31, 103,
	    -279, 671, -1731, 32546, 2006, -747, 312, -118, 37, -8, 1, 0, -1, 12, -56, 190, -516, 1246, -3167, 31887,
	    4259, -1549, 648, -248, 78, -18, 2, 0, -1, 15, -74, 257, -707, 1714, -4299, 30808, 6722, -2375, 994, -384,
	    122, -29, 4, 0, -2, 17, -87, 305, -849, 2067, -5127, 29336, 9351, -3196, 1338, -520, 169, -41, 6, 0, -2, 18,
	    -93, 334, -941, 2303, -5659, 27510, 12092, -3974, 1662, -652, 214, -53, 8, 0, -1, 17, -95, 346, -985, 2425,
	    -5912, 25375, 14888, -4673, 1951, -771, 257, -65, 10, 0, -1, 16, -92, 341, -985, 2439, -5908, 22985, 17679,
	    -5254, 2188, -871, 294, -76, 13, -1, -1, 15, -85, 323, -945, 2355, -5678, 20399, 20399, -5678, 2355, -945,
	    323, -85, 15, -1, -1, 13, -76, 294, -871, 2188, -5254, 17679, 22985, -5908, 2439, -985, 341, -92, 16, -1, 0,
	    10, -65, 257, -771, 1951, -4673, 14888, 25375, -5912, 2425, -985, 346, -95, 17, -1, 0, 8, -53, 214, -652,
	    1662, -3974, 12092, 27510, -5659, 2303, -941, 334, -93, 18, -2, 0, 6, -41, 169, -520, 1338, -3196, 9351,
	    29336, -5127, 2067, -849, 305, -87, 17, -2, 0, 4, -29, 122, -384, 994, -2375, 6722, 30808, -4299, 1714,
    ...
    public void setSampleData(final short[] inSampleData, final int inLoopStart, final int inLoopLength,
	...
	// Allocate new sample.
	final int newSampleLength = Sample.DELAY + sampleLength + (pingPong ? outLoopLength : 0) + Sample.FILTER_TAPS;
	final short[] newSampleData = new short[newSampleLength];
	System.arraycopy(outSampleData, 0, newSampleData, Sample.DELAY, sampleLength);
	...
	for (int idx = outLoopStart + outLoopLength, end = idx + Sample.FILTER_TAPS; idx < end; idx++) {
    ...
    public void resampleNearest(final int sampleIdx, final int sampleFrac, final int step, final int leftGain,
	    final int rightGain, final int[] mixBuffer, final int offset, final int length) {
    ...
    public void resampleLinear(final int sampleIdx, final int sampleFrac, final int step, final int leftGain,
	    final int rightGain, final int[] mixBuffer, final int offset, final int length) {
	final int loopLen = this.loopLength;
	final int loopEnd = this.loopStart + loopLen;
    ...
    public void resampleSinc(final int sampleIdx, final int sampleFrac, final int step, final int leftGain,
	    final int rightGain, final int[] mixBuffer, final int offset, final int length) {
	final int loopLen = this.loopLength;
	final int loopEnd = this.loopStart + loopLen;
	int outSampleIdx = sampleIdx;
	...
	while (outIdx < outEnd) {
	    ...
	    a1 >>= Sample.FP_SHIFT;
	    a2 >>= Sample.FP_SHIFT;
	    final int y = a1 + ((a2 - a1) * (outSampleFrac & Sample.TABLE_INTERP_MASK) >> Sample.TABLE_INTERP_SHIFT);
	    mixBuffer[outIdx++] += y * leftGain >> Sample.FP_SHIFT;
	    mixBuffer[outIdx++] += y * rightGain >> Sample.FP_SHIFT;
	    outSampleFrac += step;
	    outSampleIdx += outSampleFrac >> Sample.FP_SHIFT;
