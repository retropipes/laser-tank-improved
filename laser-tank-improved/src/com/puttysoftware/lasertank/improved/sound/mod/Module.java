...
public class Module {
	...
	for (int patIdx = 0; patIdx < this.numPatterns; patIdx++) {
	    final Pattern pattern = this.patterns[patIdx] = new Pattern(this.numChannels, 64);
	...
	for (int instIdx = 1; instIdx <= this.numInstruments; instIdx++) {
		...
		if (signedSamples) {
		    ...
		    for (int idx = 0, end = sampleData.length; idx < end; idx++) {
			final int sam = moduleData[sampleOffset] & 0xFF | (moduleData[sampleOffset + 1] & 0xFF) << 8;
	...
	final boolean deltaEnv = Module.isoLatin1(moduleData, 38, 20).startsWith("DigiBooster Pro");
	...
	for (int patIdx = 0; patIdx < this.numPatterns; patIdx++) {
	    ...
	    final Pattern pattern = this.patterns[patIdx] = new Pattern(this.numChannels, numRows);
	...
	for (int insIdx = 1; insIdx <= this.numInstruments; insIdx++) {
	    ...
	    final int numSamples = instrument.numSamples = Module.ushortle(moduleData, dataOffset + 27);
	    if (numSamples > 0) {
		...
		final int sampleDataBytes = Module.intle(moduleData, sampleHeaderOffset);
	...
	try {
	    final char[] str = new String(buf, offset, len, "Cp850").toCharArray();
