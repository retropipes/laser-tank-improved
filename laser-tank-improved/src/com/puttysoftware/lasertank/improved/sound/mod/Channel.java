...
public class Channel {
    ...
    private static final int[] periodTable = {
	    ...
	    54784, 54390, 53999, 53610, 53224, 52841, 52461, 52084, 51709, 51337, 50968, 50601, 50237, 49876, 49517,
	    49161, 48807, 48456, 48107, 47761, 47418, 47076, 46738, 46401, 46068, 45736, 45407, 45081, 44756, 44434,
	    44115, 43797, 43482, 43169, 42859, 42550, 42244, 41940, 41639, 41339, 41042, 40746, 40453, 40162, 39873,
	    39586, 39302, 39019, 38738, 38459, 38183, 37908, 37635, 37365, 37096, 36829, 36564, 36301, 36040, 35780,
	    35523, 35267, 35014, 34762, 34512, 34263, 34017, 33772, 33529, 33288, 33049, 32811, 32575, 32340, 32108,
	    31877, 31647, 31420, 31194, 30969, 30746, 30525, 30306, 30088, 29871, 29656, 29443, 29231, 29021, 28812,
	    28605, 28399, 28195, 27992, 27790, 27590, 27392, 27195, 26999, 26805, 26612, 26421, 26231, 26042 };
    ...
    private static final short[] arpTuning = { 4096, 4340, 4598, 4871, 5161, 5468, 5793, 6137, 6502, 6889, 7298, 7732,
	    8192, 8679, 9195, 9742 };
    private static final short[] sineTable = { 0, 24, 49, 74, 97, 120, 141, 161, 180, 197, 212, 224, 235, 244, 250, 253,
    ...
    public Channel(final Module newModule, final int id, final int newSampleRate, final GlobalVol newGlobalVol) {
    ...
    public void resample(final int[] outBuf, final int offset, final int length, final int interpolation) {
	if (this.ampl <= 0) {
	    return;
	}
	...
	case NEAREST:
	    this.sample.resampleNearest(this.sampleIdx, this.sampleFra, this.step, lAmpl, rAmpl, outBuf, offset,
		    length);
	    break;
	case LINEAR:
	default:
	    this.sample.resampleLinear(this.sampleIdx, this.sampleFra, this.step, lAmpl, rAmpl, outBuf, offset, length);
	    break;
	case SINC:
	    this.sample.resampleSinc(this.sampleIdx, this.sampleFra, this.step, lAmpl, rAmpl, outBuf, offset, length);
	    break;
	}
    }
	...
	if (this.noteKey > 0) {
		...
		final boolean isPorta = (this.noteVol & 0xF0) == 0xF0 || this.noteEffect == 0x03
			|| this.noteEffect == 0x05 || this.noteEffect == 0x87 || this.noteEffect == 0x8C;
		if (!isPorta) {
		...
		if (this.module.linearPeriods) {
		    ...
		    this.portaPeriod = this.module.c2Rate * this.portaPeriod / this.sample.c2Rate;
		}
		if (!isPorta) {
		    this.period = this.portaPeriod;
		    this.sampleIdx = this.sampleFra = 0;
