...
/**
 * An AOE type that has a center and a radius, and uses shadowcasting to create
 * a burst of rays from the center, out to the distance specified by radius. You
 * can specify the RadiusType to Radius.DIAMOND for Manhattan distance,
...
public class BurstAOE implements AOE {
    ...
    public BurstAOE(final Coord center, final int radius, final Radius radiusType) {
    ...
    public BurstAOE(final Coord center, final int radius, final Radius radiusType, final int minRange,
	    final int maxRange) {
	this.fov = new FOV(FOV.SHADOW);
	this.center = center;
	this.radius = radius;
	...
	return AreaUtils.arrayToHashMap(
		this.fov.calculateFOV(null, this.center.x, this.center.y, this.radius, this.radiusType));
    }
