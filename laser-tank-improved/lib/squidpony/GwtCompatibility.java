...
/**
 * Static methods useful to be GWT-compatible. You can think of the purpose of
 * this class as "GWT, and Compatibility". There's a replacement for a Math
 * method that isn't available on GWT, a quick way to get the first element in
 * an Iterable, and also a method to easily clone a Coord array.
 *
 * @author smelC
...
public class GwtCompatibility {
    /**
     * Gets an exact copy of an array of Coord. References are shared, which should
     * be the case for all usage of Coord since they are immutable and thus don't
     * need multiple variants on a Coord from the pool.
     *
     * @param input an array of Coord to copy
     * @return A clone of {@code input}.
     */
    public static Coord[] cloneCoords(final Coord[] input) {
	final Coord[] result = new Coord[input.length];
	// System.arraycopy, despite being cumbersome, is the fastest way to copy an
	// array on the JVM.
    ...
    /**
     * A replacement for Math.IEEEremainder, just because Math.IEEEremainder isn't
     * GWT-compatible. Gets the remainder of op / d, which can be negative if any
    ...
    public static double IEEEremainder(final double op, final double d) {
     ...
     * including end, as an int array. Delegates to ArrayTools, and using ArrayTools
     ...
     * not including end, as an int array. Delegates to ArrayTools, and using
     * ArrayTools directly is preferred.
     *
     ...
     * end, including end, as a char array. Delegates to ArrayTools, and using
     * ArrayTools directly is preferred.
     *
     ...
     * {@link #charSpan(char, char)}. Delegates to ArrayTools, and using ArrayTools
     * directly is preferred.
     *
    ...
    /**
     * Gets the first item in an Iterable of T, or null if it is empty. Meant for
     * collections like LinkedHashSet, which can promise a stable first element but
     * don't provide a way to access it. Not exactly a GWT compatibility method, but
     * more of a Java standard library stand-in. Even though LinkedHashSet does not
     * support this out-of-the-box, OrderedSet already provides a first() method.
     *
     * @param collection an Iterable of T; if collection is null or empty this
     *                   returns null
     * @param            <T> any object type
     * @return the first element in collection, or null if it is empty or null
     *         itself
     */
     ...
     * no references with source. Delegates to ArrayTools, and using ArrayTools
     * directly is preferred.
     *
     ...
     */
    @Deprecated
    public static char[][] copy2D(final char[][] source) {
	return ArrayTools.copy(source);
    }

    /**
     * Gets a copy of the 2D int array, source, that has the same data but shares no
     * references with source. Delegates to ArrayTools, and using ArrayTools
     * directly is preferred.
     *
     ...
     */
    @Deprecated
    public static int[][] copy2D(final int[][] source) {
	return ArrayTools.copy(source);
    }
     ...
     */
    @Deprecated
    public static double[][] copy2D(final double[][] source) {
	return ArrayTools.copy(source);
    }

    /**
     * Gets a copy of the 2D boolean array, source, that has the same data but
     * shares no references with source. Delegates to ArrayTools, and using
     * ArrayTools directly is preferred.
     *
     ...
     */
    @Deprecated
    public static boolean[][] copy2D(final boolean[][] source) {
	return ArrayTools.copy(source);
    }
     ...
     * position in a larger array, often freshly allocated. Delegates to ArrayTools,
     * and using ArrayTools directly is preferred.
     *
     ...
     *         position
     * @deprecated Use {@link ArrayTools#insert(char[][], char[][], int, int)}
     *             instead.
     */
    @Deprecated
    public static char[][] insert2D(final char[][] source, final char[][] target, final int x, final int y) {
     ...
     * @return a modified copy of target with source inserted into it at the given
     *         position
     * @deprecated Use {@link ArrayTools#insert(int[][], int[][], int, int)}
     *             instead.
     */
    @Deprecated
    public static int[][] insert2D(final int[][] source, final int[][] target, final int x, final int y) {
	return ArrayTools.insert(source, target, x, y);
    }
     ...
     * @return a modified copy of target with source inserted into it at the given
     *         position
     * @deprecated Use {@link ArrayTools#insert(double[][], double[][], int, int)}
     *             instead.
     */
    @Deprecated
    public static double[][] insert2D(final double[][] source, final double[][] target, final int x, final int y) {
	return ArrayTools.insert(source, target, x, y);
    }
     ...
     * @return a modified copy of target with source inserted into it at the given
     *         position
     * @deprecated Use {@link ArrayTools#insert(boolean[][], boolean[][], int, int)}
     *             instead.
     */
    @Deprecated
    public static boolean[][] insert2D(final boolean[][] source, final boolean[][] target, final int x, final int y) {
	return ArrayTools.insert(source, target, x, y);
    }

    /**
     * Creates a 2D array of the given width and height, filled with entirely with
     * the value contents. Delegates to ArrayTools, and using ArrayTools directly is
     * preferred.
     *
     ...
     */
    @Deprecated
    public static char[][] fill2D(final char contents, final int width, final int height) {
     ...
     */
    @Deprecated
    public static int[][] fill2D(final int contents, final int width, final int height) {
	return ArrayTools.fill(contents, width, height);
    }
     ...
     */
    @Deprecated
    public static double[][] fill2D(final double contents, final int width, final int height) {
	return ArrayTools.fill(contents, width, height);
    }
     ...
     */
    @Deprecated
    public static boolean[][] fill2D(final boolean contents, final int width, final int height) {
	return ArrayTools.fill(contents, width, height);
    }
     ...
     * another ordering. Delegates to ArrayTools, and using ArrayTools directly is
     * preferred.
     *
     ...
     * inverse, able to reverse the reordering and vice versa. Delegates to
     * ArrayTools, and using ArrayTools directly is preferred.
     *
     ...
     * same size as ordering passed to it as an additional argument. Delegates to
     * ArrayTools, and using ArrayTools directly is preferred.
     *
     * @param ordering the ordering to find the inverse for
