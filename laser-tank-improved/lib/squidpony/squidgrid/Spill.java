...
public class Spill {
    ...
    /**
     * The cells that are filled by the Spill when it reaches its volume or limits
     * will be true; others will be false.
     */
    public boolean[][] spillMap;
    /**
     * The list of points that the Spill will randomly fill, starting with what is
     * passed to start(), in order of when they are reached.
     */
    ...
    /**
     * The LightRNG used as a RandomnessSource for the RNG this object uses. Can
     * have its state read and set.
     */
     ...
     * using this class.
     */
    public Spill(final RNG random) {
	this.lrng = new LightRNG(random.nextLong());
	this.rng = new StatefulRNG(this.lrng);
	this.fresh = new OrderedSet<>();
    }
    ...
    /**
     * Construct a Spill without a level to actually scan. This constructor allows
     * you to specify a RandomnessSource in the form of a LightRNG, which will be
     * referenced in this class (if the state of random changes because this object
     * needed a random number, the state change will be reflected in the code that
     * passed random to here).
     *
     * If you use this constructor, you must call an initialize() method before
     * using this class.
     */
    public Spill(final LightRNG random) {
	this.lrng = random;
	this.rng = new StatefulRNG(this.lrng);
	this.fresh = new OrderedSet<>();
    }
     ...
     * @param measurement
     */
    public Spill(final boolean[][] level, final Measurement measurement) {
	this.lrng = new LightRNG();
	this.rng = new StatefulRNG(this.lrng);
     ...
     * @param level
     */
    public Spill(final char[][] level, final char alternateWall) {
	this.lrng = new LightRNG();
	this.rng = new StatefulRNG(this.lrng);
     ...
     * @param measurement
     */
    public Spill(final char[][] level, final Measurement measurement) {
	this.lrng = new LightRNG();
	this.rng = new StatefulRNG(this.lrng);
	this.measurement = measurement;
	this.initialize(level);
     ...
     * constructor specifies a distance measurement.
     *
     * This constructor allows you to specify an RNG, but the actual
     * RandomnessSource the RNG that this object uses will not be identical to the
     * one passed as random (64 bits will be requested from the passed RNG, and that
     * will be used to seed this class' RNG).
     *
     * @param level
     * @param measurement
     */
    public Spill(final char[][] level, final Measurement measurement, final RNG random) {
	this.lrng = new LightRNG(random.nextLong());
	this.rng = new StatefulRNG(this.lrng);
	this.measurement = measurement;
	this.initialize(level);
     ...
     * constructor specifies a distance measurement.
     *
     * This constructor allows you to specify a RandomnessSource in the form of a
     * LightRNG, which will be referenced in this class (if the state of random
     * changes because this object needed a random number, the state change will be
     * reflected in the code that passed random to here).
     *
     * @param level
     * @param measurement
     */
    public Spill(final char[][] level, final Measurement measurement, final LightRNG random) {
	this.lrng = random;
	this.rng = new StatefulRNG(this.lrng);
	this.measurement = measurement;
	this.initialize(level);
     ...
     * @return
     */
    public Spill initialize(final char[][] level, final char alternateWall) {
	this.fresh = new OrderedSet<>();
	this.width = level.length;
	this.height = level[0].length;
	this.spillMap = new boolean[this.width][this.height];
	...
	this.spillMap[pt.x][pt.y] = false;
    }

    protected void setFresh(final int x, final int y) {
	if (!this.initialized) {
	    return;
	}
	this.fresh.add(Coord.get(x, y));
    }
     ...
     * to entry will be true, the cells near that will be true if chosen at random
     * from all passable cells adjacent to a filled (true) cell, and all other cells
     * will be false. This takes a total number of cells to attempt to fill (the
     * volume parameter), and will fill less if it has completely exhausted all
     * passable cells. If the measurement this Spill uses is anything other than
     * MANHATTAN, you can expect many gaps in the first filled area. Subsequent
     * calls to start() with the same entry and a higher volume will expand the area
     * of the Spill, and are likely to fill any gaps after a few subsequent calls.
     * Increasing the volume slowly is the best way to ensure that gaps only exist
     * on the very edge if you use a non-MANHATTAN measurement.
     *
     * @param entry      The first cell to spread from, which should really be
     *                   passable.
     * @param volume     The total number of cells to attempt to fill, which must be
     *                   non-negative.
     * @param impassable A Set of Position keys representing the locations of moving
     *                   obstacles to a path that cannot be moved through; this can
     *                   be null if there are no such obstacles.
     ...
     *         completely.
     */
    public ArrayList<Coord> start(final Coord entry, final int volume, Set<Coord> impassable) {
	if (!this.initialized) {
	    return null;
	}
	if (impassable == null) {
	...
	if (!this.physicalMap[entry.x][entry.y] || impassable.contains(entry)) {
	    return null;
	}
	this.spreadPattern = new ArrayList<>(volume);
	this.spillMap[entry.x][entry.y] = true;
	Coord temp;
	for (int x = 0; x < this.spillMap.length; x++) {
	    for (int y = 0; y < this.spillMap[x].length; y++) {
		temp = Coord.get(x, y);
		if (this.spillMap[x][y] && !impassable.contains(temp)) {
		    this.fresh.add(temp);
		}
	    }
	}
	final Direction[] dirs = this.measurement == Measurement.MANHATTAN ? Direction.CARDINALS : Direction.OUTWARDS;
	while (!this.fresh.isEmpty() && this.spreadPattern.size() < volume) {
	    final Coord cell = this.fresh.randomItem(this.rng);// toArray(new
							       // Coord[fresh.size()])[rng.nextInt(fresh.size())];
	    this.spreadPattern.add(cell);
	    this.spillMap[cell.x][cell.y] = true;
	    for (final Direction dir : dirs) {
		final Coord adj = cell.translate(dir.deltaX, dir.deltaY);
		final double h = this.heuristic(dir);
		if (this.physicalMap[adj.x][adj.y] && !this.spillMap[adj.x][adj.y] && !impassable.contains(adj)
			&& this.rng.nextDouble() <= 1.0 / h) {
		    this.setFresh(adj);
		}
	    }
	    this.fresh.remove(cell);
	}
