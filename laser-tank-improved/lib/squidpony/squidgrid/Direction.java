...
public enum Direction {
    ...
    /**
     * An array which holds only the four cardinal directions in counter-clockwise
     * order.
     */
    ...
    /**
     * An array which holds all eight OUTWARDS directions in counter-clockwise
     * order.
     */
    public static final Direction[] COUNTERCLOCKWISE = { UP, UP_LEFT, LEFT, DOWN_LEFT, DOWN, DOWN_RIGHT, RIGHT,
	    UP_RIGHT };
    /**
     ...
     *
     * This can be used to get the primary magnitude intercardinal direction from an
    ...
    /**
     * Gets an estimate at the correct direction that a position lies in given the
     * distance towards it on the x and y axes. If x and y are both between -1 and 1
     * inclusive, this will always be accurate, and should be faster than
     * {@link #getDirection(int, int)} by avoiding trigonometry or any other math on
     * doubles. If at least one of x or y is 0, then this will also be accurate and
     * will produce either a cardinal direction or NONE if both are 0. If x and y
     * are both non-zero, this will always produce a diagonal, even if a cardinal
     * direction should be more accurate; this behavior may sometimes be desirable
     * to detect when some position is even slightly off from a true cardinal
     * direction.
     *
     ...
     * @return the Direction that x,y lies in, roughly; will always be accurate for
     *         arguments between -1 and 1 inclusive
     */
     ...
     * @return
     */
    public static Direction getCardinalDirection(final int x, final int y) {
	if (x == 0 && y == 0) {
	    return NONE;
	}
     ...
     * @param to   The desired point to reach.
     * @return The direction to follow to go from {@code from} to {@code to}. It can
     *         be cardinal or diagonal.
     */
    public static Direction toGoTo(final Coord from, final Coord to) {
     ...
     *
     * If considering only Cardinal directions, calling this twice will get the next
     * clockwise cardinal direction.
     *
     * @return
     */
     ...
     *
     * If considering only Cardinal directions, calling this twice will get the next
     * counterclockwise cardinal direction.
     *
     * @return
     */
