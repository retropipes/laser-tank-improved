...
/**
 * A variant on {@link MixedGenerator} that creates bi-radially symmetric maps
 * (basically a yin-yang shape). Useful for strategy games and possibly
 * competitive multi-player games. The Coords passed to constructors as room
 * positions do not necessarily need to be
 *
 * Created by Tommy Ettinger on 11/20/2015.
 */
public class SymmetryDungeonGenerator extends MixedGenerator {
    public static OrderedMap<Coord, List<Coord>> removeSomeOverlap(final int width, final int height,
	    final List<Coord> sequence) {
	final List<Coord> s2 = new ArrayList<>(sequence.size());
	for (final Coord c : sequence) {
	    if (c.x * 1.0 / width + c.y * 1.0 / height <= 1.0) {
		s2.add(c);
	    }
	}
	return SymmetryDungeonGenerator.listToMap(s2);
    }

    public static OrderedMap<Coord, List<Coord>> removeSomeOverlap(final int width, final int height,
	    final Map<Coord, List<Coord>> connections) {
	final OrderedMap<Coord, List<Coord>> om2 = new OrderedMap<>(connections.size());
	final Set<Coord> keyset = connections.keySet(), newkeys = new OrderedSet<>(connections.size());
	for (final Coord c : keyset) {
	    if (c.x * 1.0 / width + c.y * 1.0 / height <= 1.0) {
		newkeys.add(c);
	    }
	}
	final Coord[] keys = newkeys.toArray(new Coord[newkeys.size()]);
	for (int i = 0; i < keys.length; i++) {
	    final Coord c = keys[i];
	    if (c.x * 1.0 / width + c.y * 1.0 / height <= 1.0) {
		final List<Coord> cs = new ArrayList<>(4);
		for (final Coord c2 : connections.get(c)) {
     ...
     */
    public SymmetryDungeonGenerator(final int width, final int height, final RNG rng) {
	this(width, height, rng, MixedGenerator.basicPoints(width, height, rng));
    }
     ...
     */
    public SymmetryDungeonGenerator(final int width, final int height, final RNG rng, final List<Coord> sequence) {
	this(width, height, rng, SymmetryDungeonGenerator.listToMap(sequence), 1f);
    }

    /**
     * This prepares a map generator that will generate a map with the given width
     * and height, using the given RNG. This version of the constructor uses a
     * LinkedHashMap with Coord keys and Coord array values to determine a branching
     * path for the dungeon to take; each key will connect once to each of the
     * Coords in its value, and you usually don't want to connect in both
     * directions. You call the different carver-adding methods to affect what the
     * dungeon will look like, putCaveCarvers(), putBoxRoomCarvers(), and
     * putRoundRoomCarvers(), defaulting to only caves if none are called. You call
     * generate() after adding carvers, which returns a char[][] for a map.
     *
     * @param width       the width of the final map in cells
     * @param height      the height of the final map in cells
     ...
     */
    public SymmetryDungeonGenerator(final int width, final int height, final RNG rng,
	    final OrderedMap<Coord, List<Coord>> connections) {
     ...
     */
    public SymmetryDungeonGenerator(final int width, final int height, final RNG rng,
	    final OrderedMap<Coord, List<Coord>> connections, final float roomSizeMultiplier) {
	super(width, height, rng, SymmetryDungeonGenerator.crossConnect(width, height, connections),
		roomSizeMultiplier);
    }

    protected static OrderedMap<Coord, List<Coord>> listToMap(final List<Coord> sequence) {
	final OrderedMap<Coord, List<Coord>> conns = new OrderedMap<>(sequence.size() - 1);
	for (int i = 0; i < sequence.size() - 1; i++) {
	    final Coord c1 = sequence.get(i), c2 = sequence.get(i + 1);
	    final List<Coord> cs = new ArrayList<>(1);
	    cs.add(c2);
	    conns.put(c1, cs);
	}
	return conns;
    ...
    protected static OrderedMap<Coord, List<Coord>> crossConnect(final int width, final int height,
	    final Map<Coord, List<Coord>> connections) {
	final OrderedMap<Coord, List<Coord>> conns = new OrderedMap<>(connections.size());
	for (final Map.Entry<Coord, List<Coord>> entry : connections.entrySet()) {
	...
	for (final List<Coord> left : connections.values()) {
	    for (final List<Coord> right : connections.values()) {
		for (final Coord lc : left) {
		    for (final Coord rc : right) {
			final Coord rc2 = Coord.get(width - 1 - rc.x, height - 1 - rc.y);
			final double dist = lc.distance(rc2);
			if (dist < 0.001) {
			    continue;
			}
			if (dist < lowest1) {
			    ...
			    l2 = lc;
			    r2 = rc2;
			} else if (dist < lowest3 && !lc.equals(l1) && !rc2.equals(r1) && !lc.equals(l2)
				&& !rc2.equals(r2)) {
			    lowest3 = dist;
			    l3 = lc;
			    r3 = rc2;
			} else if (dist < lowest4 && !lc.equals(l1) && !rc2.equals(r1) && !lc.equals(l2)
				&& !rc2.equals(r2) && !lc.equals(l3) && !rc2.equals(r3)) {
			    lowest4 = dist;
			    l4 = lc;
			    r4 = rc2;
			}
	...
	return super.mark(x, y) || super.mark(this.width - 1 - x, this.height - 1 - y);
    }
	...
	super.markPiercing(this.width - 1 - x, this.height - 1 - y);
    }
	...
	super.wallOff(x, y);
	super.wallOff(this.width - 1 - x, this.height - 1 - y);
    }
	...
	super.markEnvironment(this.width - 1 - x, this.height - 1 - y, kind);
    }
}
