...
public class Symbol {
     ...
     * <p>
     */
    public static final int NOTHING = Integer.MIN_VALUE, START = -1, GOAL = -2, BOSS = -3, SWITCH_ON = -4, // used as a
													   // condition
													   // (lock)
	...
	return value == Symbol.SWITCH_ON || value == Symbol.SWITCH_OFF;
    }

    /**
     * Like {@link #toString(int)}, but returns one char instead, sometimes using
     * some roguelike map conventions:
     * <ul>
     ...
     * <li>Any number between 0 and 255 inclusive is mapped to a letter using
     * {@link ArrayTools#letterAt(int)}</li>
     * <li>Anything else maps to {@code '*'}</li>
     * </ul>
     *
     * @param value a symbol int that should be less than 256, and if negative
     *              should equal one of the Symbol constants
     * @return a single char that can be used to identify value
     */
    public static char asChar(final int value) {
	if (value == Symbol.START) {
	    return '<';
	} else if (value == Symbol.GOAL) {
	    return '>';
	} else if (value == Symbol.BOSS) {
	    return '!';
	} else if (value == Symbol.SWITCH_ON) {
	    return '1';
	} else if (value == Symbol.SWITCH_OFF) {
	    return '0';
	} else if (value == Symbol.SWITCH) {
	    return '&';
	} else if (value == Symbol.NOTHING) {
	    return ' ';
    ...
    /**
     * Gets a printable String representation of the int (the parameter value) that
     * represents a symbol. START, GOAL, and BOSS map to "Start", "Goal", and
     * "Boss", respectively. SWITCH, SWITCH_ON, and SWITCH_OFF map to "SW", "ON",
     * and "OFF", respectively. NOTHING maps to "NO". Any number between 0 and 255
     * inclusive is mapped to a letter using {@link ArrayTools#letterAt(int)}, and
     * that becomes the returned String. Any other number is simply printed as a
     * normal integer.
     *
     * @param value a symbol int that should be less than 256, and if negative
     *              should equal one of the Symbol constants
