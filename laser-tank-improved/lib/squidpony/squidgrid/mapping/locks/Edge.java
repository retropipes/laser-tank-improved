...
public class Edge {
     ...
     */
    public Edge(final int targetRoomId) {
	this(targetRoomId, Symbol.NOTHING);
    }

    /**
     * Creates an Edge that requires a particular Symbol to be collected before it
     * may be used by the player to travel between the Rooms.
     *
     * @param symbol the symbol that must be obtained
     */
    public Edge(final int targetRoomId, final int symbol) {
	...
	return this.symbol != Symbol.NOTHING;
    }

    /**
     * @return the symbol that must be obtained to pass along this edge or null if
     *         there are no required symbols
     */
	...
	if (other instanceof Edge) {
	    ...
	    return this.targetRoomId == o.targetRoomId && this.symbol == o.symbol;
	} else {
