...
/**
 * Generate dungeons with between 1 and 3 primary "lanes" going from the upper
 * left "base" to the bottom right "base" (and vice versa, since this is
 * symmetrical). Also fills the area not covered by lanes with "jungle" (random,
 * but symmetrical, room or cave connections). Dungeons are produced by
 * MixedGenerator, like those SerpentMapGenerator makes, but include the wide
 * lanes going from corner to corner. You can call different methods like
 * putCaveCarvers(), putBoxRoomCarvers(), putWalledRoundRoomCarvers(), etc. to
 * affect the "jungle", which defaults to caves unless one or more of the
 * putXXXCarvers methods was called. The lanes are always 5 floor cells wide,
 * measured 8-way. This supports the getEnvironment() method, which can be used
 * in conjunction with RoomFinder to find where separate room, corridor, and
 * cave areas have been placed. <br>
 * A preview can be seen here
 * https://gist.github.com/tommyettinger/4f57cff23eead11b17bf , with dungeons
 * created with one, two, and three lanes, and only using box-shaped rooms for
 * "jungle." Currently, the two-lane dungeon seems to be ideal for maps that
 * aren't incredibly large; the samples are 80x80, but larger maps may have
 * better jungle layout with three lanes than those three-lane maps can manage
 * on smaller sizes. Another potential advantage of the two-lane approach is
 * that it can be used to generate a "ring" of wide paths around a central
 * "core" of jungle, which wasn't originally intended as a use of this generator
 * but could be very useful for games that, for instance, want guards patrolling
 * an obvious ring, while the player, monsters, and/or other prisoners start in
 * the jungle. Created by Tommy Ettinger on 10/24/2015.
 */
public class LanesMapGenerator implements IDungeonGenerator {
     ...
     * and height, using the given RNG. The dungeon will have the specified number
     * of wide lanes going from upper left to lower right, possibly taking a longer
     * path to approach the other corners. You call the different carver-adding
     * methods to affect what the non-lane portion of the dungeon will look like,
     ...
     *               random choices.
     * @param lanes  between 1 and 3; the number of wide paths to generate going
     *               from upper left to lower right.
     * @see MixedGenerator
     */
    public LanesMapGenerator(final int width, final int height, final RNG rng, final int lanes) {
	if (width <= 8 || height <= 8) {
	...
	this.random = rng;
	/*
	 * long columnAlterations = random.nextLong(); float columnBase = width /
	 * (Long.bitCount(columnAlterations) + 16.0f); long rowAlterations =
	 * random.nextLong(); float rowBase = height / (Long.bitCount(rowAlterations) +
	 * 16.0f);
	 *
	 * columns = new int[32]; rows = new int[32]; int csum = 0, rsum = 0; long b =
	 * 3; for (int i = 0; i < 32; i++, b <<= 2) { columns[i] = csum +
	 * (int)(columnBase * 0.5f * (1 + Long.bitCount(columnAlterations & b))); csum
	 * += (int)(columnBase * (1 + Long.bitCount(columnAlterations & b))); rows[i] =
	 * rsum + (int)(rowBase * 0.5f * (1 + Long.bitCount(rowAlterations & b))); rsum
	 * += (int)(rowBase * (1 + Long.bitCount(rowAlterations & b))); } int cs =
	 * (width - csum); int rs = (height - rsum); int cs2 = cs, rs2 = rs, cs3 = cs,
	 * rs3 = rs; for (int i = 15; i >= 0; i--) { cs2= cs2 * i / 15; rs2 = rs2 * i /
	 * 15; columns[i] -= cs2; rows[i] -= rs2; } for (int i = 15; i >= 16; i--) { cs3
	 * = cs3 * (i - 16) / 16; rs3 = rs3 * (i - 16) / 16; columns[i] += cs3; rows[i]
	 * += rs3; }
	 */
	...
	Coord temp, t;
	int m = this.random.nextInt(32), r = this.random.between(8, 24);
	temp = CoordPacker.hilbertToCoord(m);
	final Coord starter = CoordPacker.hilbertToCoord(m);
	m += r;
	for (int i = r; i < 256 && m < 256 - 9; i += r, m += r) {
	    ...
	    temp = CoordPacker.hilbertToCoord(m);
	    r = this.random.between(8, 24);
	    for (int j = 0, p = r - 1; j < 3 && p > 2
		    && Math.min(this.random.nextDouble(), this.random.nextDouble()) < 0.2; j++, p -= this.random
			    .between(1, p)) {
	...
	this.mix = new SymmetryDungeonGenerator(width, height, this.random, connections, 0.6f);
	final boolean[][] fixed = new boolean[width][height];
	if (lanes != 2) {
	    final List<Coord> path = DDALine.line(3, 3, width - 4, height - 4);
	...
	if (lanes > 1) {
	    final List<Coord> path = DDALine.line(3, 3, 3, height - 4);
     ...
     * same size as the last char 2D array prduced by generate(), and the value of
     * this method if called before generate() is undefined, but probably will be a
     * 2D array of all 0 (UNTOUCHED).
     * <ul>
