...
/**
 * Use FOV instead, probably with type SHADOW; this has the same results as
 * shadowcasting FOV but is slower, currently. This will probably be rewritten
 * in a very different way. <br>
 * A different kind of FOV implementation that pre-processes the map to better
 * analyze the locations of corners. Based loosely on Adam Milazzo's FOV,
 * http://www.adammil.net/blog/v125_Roguelike_Vision_Algorithms.html , which in
...
public class BevelFOV extends FOV {
    ...
    /**
     * Included for compatibility; ignores the parameter and calls the default
     * parameter with no argument.
     *
     * @param type ignored
     */
    public BevelFOV(final int type) {
	this();
    }
	...
	this.maybeCache(resistanceMap);
	for (final Direction d : Direction.DIAGONALS) {
	    this.shadowCast(1, 1.0, 0.0, 0, d.deltaX, d.deltaY, 0, rad, startX, startY, decay, radiusTechnique);
	    this.shadowCast(1, 1.0, 0.0, d.deltaX, 0, 0, d.deltaY, rad, startX, startY, decay, radiusTechnique);
	}
	return this.light;
    }
	...
	for (final Direction d : FOV.ccw) {
		...
		this.light = this.shadowCastLimited(1, 1.0, 0.0, 0, d.deltaX, d.deltaY, 0, rad, startX, startY, decay,
			radiusTechnique, angle2, span2);
		this.light = this.shadowCastLimited(1, 1.0, 0.0, d.deltaX, 0, 0, d.deltaY, rad, startX, startY, decay,
			radiusTechnique, angle2, span2);
	    }
	}
	return this.light;
     ...
     * sources to those visible from the starting point.
     *
     * @param resistanceMap the grid of cells to calculate on; the kind made by
     *                      DungeonUtility.generateResistances()
	...
	if (this.resMapCache != resistanceMap) {
	    this.storeMap(resistanceMap);
	}
    }

    /**
     * Pre-calculates some time-intensive calculations that generally only need to
     * be done once per map. You should pass the same exact value for resistanceMap
     * to {@link #calculateFOV(double[][], int, int)} or its overloads if you want
     * to avoid re-calculating this map analysis. If an element of resistanceMap
     * changes, you should call this method again before you call calculateFOV,
     * though it can wait until you've made all the changes you want to
     * resistanceMap's elements. This class does not have a way to tell when the
     * array has been modified in user code, so you should take care if you change
     * resistanceMap to call this method.
     *
     * @param resistanceMap a 2D double array, as produced by
	...
	if (this.resMapCache == null || this.resMapCache.length != resistanceMap.length
		|| this.resMapCache[0].length != resistanceMap[0].length) {
	    this.resMapCache = resistanceMap;
	    if (this.map == null) {
		this.map = new GreasedRegion(resistanceMap, 1.0, Double.POSITIVE_INFINITY, 3).removeCorners();
	    } else {
		this.map.refill(resistanceMap, 1.0, Double.POSITIVE_INFINITY, 3).removeCorners();
	    }
	} else {
	    for (int x = 0; x < resistanceMap.length; x++) {
		for (int y = 0; y < resistanceMap[x].length; y++) {
		    if (this.resMapCache[x][y] != resistanceMap[x][y]) {
			this.resMapCache = resistanceMap;
			if (this.map == null) {
			    this.map = new GreasedRegion(resistanceMap, 1.0, Double.POSITIVE_INFINITY, 3)
				    .removeCorners();
			} else {
			    this.map.refill(resistanceMap, 1.0, Double.POSITIVE_INFINITY, 3).removeCorners();
			}
    ...
    private double[][] shadowCast(final int row, double start, final double end, final int xx, final int xy,
	    final int yx, final int yy, final double radius, final int startx, final int starty, final double decay,
	    final Radius radiusStrategy) {
	double newStart = 0;
	if (start < end) {
	    return this.light;
	}
	final int width = this.light.length;
	final int height = this.light[0].length;
	...
	for (int distance = row; distance <= radius && distance < width + height && !blocked; distance++) {
		...
		if (deltaRadius <= radius) {
		    this.light[currentX][currentY] = 1 - decay * deltaRadius;
		}
		if (blocked) { // previous cell was a blocking one
		    if (this.map.contains(currentX * 3 + 1 + xx + xy, currentY * 3 + 1 + yx + yy)) { // hit a wall
			newStart = rightSlope;
		    } else if (!this.map.contains(currentX * 3 + 1, currentY * 3 + 1)) {
			blocked = false;
			start = newStart;
		    }
		} else {
		    if (distance < radius && this.map.contains(currentX * 3 + 1, currentY * 3 + 1)) { // hit a wall
												      // within
			// sight line
			blocked = true;
			this.light = this.shadowCast(distance + 1, start, leftSlope, xx, xy, yx, yy, radius, startx,
				starty, decay, radiusStrategy);
			newStart = rightSlope;
		    }
		}
	    }
    ...
    private double[][] shadowCastLimited(final int row, double start, final double end, final int xx, final int xy,
	    final int yx, final int yy, final double radius, final int startx, final int starty, final double decay,
	    final Radius radiusStrategy, final double angle, final double span) {
	double newStart = 0;
	if (start < end) {
	    return this.light;
	}
	...
	for (int distance = row; distance <= radius && distance < width + height && !blocked; distance++) {
		...
		final double newAngle = Math.atan2(currentY - starty, currentX - startx) + Math.PI * 2;
		...
		if (blocked) { // previous cell was a blocking one
		    ...
		    if (this.map.contains(currentX * 3 + 1 + xx + xy, currentY * 3 + 1 + yx + yy)
			    && distance < radius) {// hit
			// a
			// wall
			// within
			...
			// line
			blocked = true;
			this.light = this.shadowCastLimited(distance + 1, start, leftSlope, xx, xy, yx, yy, radius,
				startx, starty, decay, radiusStrategy, angle, span);
			newStart = rightSlope;
		    }
		}
	    }
