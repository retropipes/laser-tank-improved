...
public class Matcher implements MatchResult {
     ...
     */
    Matcher(final Pattern parent, final CharSequence text) {
	this.pattern = parent;
	...
	public int start(final int group) {
	    return this.m.start(group);
	}

	@Override
	public int start(final String name) {
	    return this.m.start(name);
	}
	...
	public int end(final int group) {
	    return this.m.end(group);
	}

	@Override
	public int end(final String name) {
	    return this.m.end(name);
	}
	...
	public String group(final int group) {
	    return this.m.group(group);
	}

	@Override
	public String group(final String name) {
	    return this.m.group(name);
	}
    ...
    /**
     * Returns the input subsequence matched by the previous match. <br>
     ...
     * are equivalent. <br>
     * Capturing groups are indexed from left to right, starting at one. Group zero
     * denotes the entire pattern, so the expression <tt>m.group(0)</tt> is
     * equivalent to <tt>m.group()</tt>. <br>
    ...
    /**
     * Returns the number of capturing groups in this match result's pattern. <br>
	...
	final int limit = this.matcher.targetEnd();
	...
	return this.matcher.search(regexodus.Matcher.ACCEPT_INCOMPLETE);
    }
     ...
     * group(g)} respectively. For <tt>$</tt><i>g</i><tt></tt>, the first number
     ...
     * @return This matcher
     *
     * @throws IllegalStateException     If no match has yet been attempted, or if
     *                                   the previous match operation failed
     *
     * @throws IllegalArgumentException  If the replacement string refers to a
     *                                   named-capturing group that does not exist
     *                                   in the pattern
     *
     * @throws IndexOutOfBoundsException If the replacement string refers to a
     *                                   capturing group that does not exist in the
     *                                   pattern
     */
	...
	this.matcher.getGroup(regexodus.MatchResult.TARGET, Replacer.wrap(sb));
	return sb;
    }
     ...
     * with the given replacement string. <br>
     * This method first resets this matcher. It then scans the input sequence
     ...
     * result by the replacement string. The replacement string may contain
     * references to captured subsequences. <br>
     * Note that backslashes (<tt>\</tt>) and dollar signs (<tt>$</tt>) in the
     * replacement string may cause the results to be different than if it were
     * being treated as a literal replacement string. Dollar signs may be treated as
     * references to captured subsequences as described above, and backslashes are
     * used to escape literal characters in the replacement string. <br>
     * Given the regular expression <tt>dog</tt>, the input
     * <tt>"zzzdogzzzdogzzz"</tt>, and the replacement string <tt>"cat"</tt>, an
     * invocation of this method on a matcher for that expression would yield the
     * string <tt>"zzzcatzzzdogzzz"</tt>. <br>
     * Invoking this method changes this matcher's state. If the matcher is to be
     * used in further matching operations then it should first be reset.
     *
     * @param replacement The replacement string
