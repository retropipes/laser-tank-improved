...
public class Pattern implements Serializable {
     ...
     *
     * <p>
     * Unicode-aware case-insensitive matching is always enabled by this flag,
     * regardless of unicode flag status.
     *
     * <p>
     ...
     * end of the entire input sequence. <br>
     * Multiline mode can also be enabled via the embedded flag
     * expression&nbsp;<tt>(?m)</tt>.
     */
    public static final int MULTILINE = 0x08;
    /**
     * Enables literal mode. <br>
     * In literal mode, metacharacters are not interpreted at all, and they match
     * the exact, literal string used for the Pattern. This is done by running
     * {@link Pattern#quote(String)} on the regexp string when this flag is
     * specified.
     */
    public static final int LITERAL = 0x10;
    /**
     * Enables dotall mode. <br>
     ...
     * <i>POSIX character classes</i>. <br>
    ...
    /**
     * Splits the given input sequence around matches of this pattern. <br>
     ...
     * element, namely the input sequence in string form. <br>
	...
	final regexodus.Matcher m = new regexodus.Matcher(this.internal, input);
     ...
     * not included in the resulting array. <br>
     * The input <tt>"boo:and:foo"</tt>, for example, yields the following results
    ...
    private Pattern(final String p, final int flags) {
	int fm = (flags & Pattern.CASE_INSENSITIVE) != 0 ? REFlags.IGNORE_CASE : 0;
	this.flags = flags;
	fm |= (flags & Pattern.DOTALL) != 0 ? REFlags.DOTALL : 0;
	fm |= (flags & Pattern.COMMENTS) != 0 ? REFlags.IGNORE_SPACES : 0;
	fm |= (flags & Pattern.MULTILINE) != 0 ? REFlags.MULTILINE : 0;
	fm |= (flags & Pattern.UNICODE_CHARACTER_CLASS) != 0 ? REFlags.UNICODE : 0;
	if ((flags & Pattern.LITERAL) != 0) {
	    this.internal = regexodus.Pattern.compile(Pattern.quote(p), fm);
	} else {
	    this.internal = regexodus.Pattern.compile(p, fm);
	}
    }
}
