...
/**
 * A type-specific hash map with a fast, small-footprint implementation. <br>
...
public class CharCharMap implements java.io.Serializable, Cloneable {
	...
	this(expected, CharCharMap.DEFAULT_LOAD_FACTOR);
    }
     ...
     */
    public CharCharMap() {
	this(CharCharMap.DEFAULT_INITIAL_SIZE, CharCharMap.DEFAULT_LOAD_FACTOR);
    }
	...
	this(k, v, CharCharMap.DEFAULT_LOAD_FACTOR);
    }
	...
	this.size--;
	this.shiftKeys(pos);
	if (this.size < this.maxFill / 4 && this.n > CharCharMap.DEFAULT_INITIAL_SIZE) {
	...
	if (k == (char) 0) {
	    ...
	    // The starting point.
	    if (!((curr = key[pos = HashCommon.mix(k) & this.mask]) == (char) 0)) {
		if (curr == k) {
		    return pos;
		}
		while (!((curr = key[pos = pos + 1 & this.mask]) == (char) 0)) {
		    if (curr == k) {
			return pos;
		    }
		}
	...
	while (true) {
	    if ((curr = key[pos = pos + 1 & this.mask]) == (char) 0) {
		return this.defRetValue;
	    }
	    if (k == curr) {
		return this.removeEntry(pos);
     ...
     *
     * <br>To increase object reuse, this method does not change the table size. If
    ...
    /**
     * Rehashes the map, making the table as small as possible. <br>
     ...
     * optimize access speed and size. <br>
     ...
     */
    public boolean trim() {
	final int l = CharCharMap.arraySize(this.size, this.f);
	if (l >= this.n || this.size > CharCharMap.maxFill(l, this.f)) {
	    return true;
	}
	try {
	    this.rehash(l);
	} catch (final Error cantDoIt) {
	    return false;
	}
	return true;
    }
    ...
    /**
     * Rehashes this map if the table is too large. <br>
    ...
    /**
     * Returns a deep copy of this map. <br>
     * This method performs a deep copy of this hash map, but with primitive keys
     * and values it doesn't matter much.
     *
     * @return a deep copy of this map.
     */
    @Override
    public CharCharMap clone() {
	final char[] k = new char[this.key.length], v = new char[this.value.length];
    ...
    /**
     * Returns a hash code for this map. <br>
	...
	/**
	 * Quickly mixes the bits of an integer. <br>
	 ...
	 * slightly worse behaviour than murmurHash3 (in open-addressing hash tables the
	...
	public final static int mix(final int x) {
	    final int h = x * HashCommon.INT_PHI;
	    return h ^ h >>> 16;
	}
