 ...
 * polymorphism to avoid (un)boxing. <br>
 ...
 * backing array is exposed by the {@link #elements()} method. <br>
 ...
 * instead of expensive loops. <br>
 * Adapted from Sebastiano Vigna's FastUtil code,
 * https://github.com/vigna/fastutil
 *
 * @see java.util.ArrayList
 */
public class CharArrayList implements RandomAccess, Cloneable, java.io.Serializable, List<Character>,
    ...
    /**
     * Creates a new array list using a given array. <br>
	...
	this(CharArrayList.DEFAULT_INITIAL_CAPACITY);
    }
    ...
    /**
     * Creates a new array list and fills it with the elements of a given array or
     * vararg.
     *
     * @param a an array or vararg whose elements will be used to fill the array
     *          list.
     */
     ...
     */
    public CharArrayList(final String s) {
	this(s.toCharArray());
    }
    ...
    /**
     * Wraps a given array into an array list of given size. <br>
    ...
    /**
     * Wraps a given array into an array list. <br>
     * Note it is guaranteed that the type of the array returned by
     * {@link #elements()} will be the same (see the comments in the class
     * documentation).
     *
	...
	if (index >= this.size) {
	    throw new IndexOutOfBoundsException(
		    "Index (" + index + ") is greater than or equal to list size (" + this.size + ")");
	}
	return this.a[index];
    }
	...
	if (size > this.size) {
	    Arrays.fill(this.a, this.size, size, (char) 0);
	}
	this.size = size;
    }
    ...
    /**
     * Trims the backing array if it is too large. <br>
     ...
     * between <code>n</code> and {@link #size()}. <br>
	...
	public CharListIterator(final int idx) {
	    this.index = idx;
	}
	...
	int index, pos = this.index, last = -1;
	...
	/**
	 * This method just iterates the type-specific version of {@link #next()} for at
	 * most <code>n</code> times, stopping if {@link #hasNext()} becomes false.
	 */
     ...
     * class can be applied to a subList. <br>
    ...
    /**
     * Compares this type-specific array list to another one. <br>
    ...
    /**
     * Compares this array list to another array list. <br>
     * This method exists only for sake of efficiency. The implementation inherited
     * from the abstract implementation would already work.
     *
	...
	if (l instanceof CharArrayList) {
	    final CharListIterator i1 = this.listIterator(), i2 = ((CharArrayList) l).listIterator();
	    int r;
	    char e1, e2;
	...
	final ListIterator<? extends Character> i1 = this.listIterator(), i2 = l.listIterator();
	int r;
	while (i1.hasNext() && i2.hasNext()) {
    ...
    @Override
    public int hashCode() {
	final CharListIterator i = this.iterator();
	int h = 1, s = this.size();
	while (s-- != 0) {
    ...
    /**
     * Ensures that the given index is non-negative and not greater than the list
     * size.
     *
	...
	if (index > this.size()) {
	    throw new IndexOutOfBoundsException(
		    "Index (" + index + ") is greater than list size (" + this.size() + ")");
	}
    }
     ...
     * type of the specified array and the size of this list. <br>
     ...
     * list does not contain any null elements.) <br>
     ...
     * circumstances, be used to save allocation costs. <br>
	 ...
	 * @return <code>array</code>, if it can contain <code>length</code> entries or
	 *         more; otherwise, an array with <code>length</code> entries whose
	 *         first <code>preserve</code> entries are the same as those of
	 *         <code>array</code>.
	 */
	 ...
	 * length multiplied by two, provided that the given length is larger than the
	 * current length, preserving just a part of the array. <br>
	 ...
	 *         max(<code>length</code>,<code>array.length</code>/&phi;) entries
	 *         whose first <code>preserve</code> entries are the same as those of
	 *         <code>array</code>.
	 */
	public static char[] grow(final char[] array, final int length, final int preserve) {
	    if (length > array.length) {
		final int newLength = (int) Math.max(Math.min(2L * array.length, CharArrays.MAX_ARRAY_SIZE), length);
	...
	/**
	 * Ensures that a range given by an offset and a length fits an array. <br>
	 ...
	 * @throws ArrayIndexOutOfBoundsException if <code>offset</code> is negative or
	 *                                        <code>offset</code>+<code>length</code>
	 ...
	 * elements fits an array of given length. <br>
	 * This method may be used whenever an array range check is needed.
	 *
	 * @param arrayLength an array length.
	 * @param from        a start index (inclusive).
	 * @param to          an end index (inclusive).
	 * @throws IllegalArgumentException       if <code>from</code> is greater than
	 *                                        <code>to</code>.
	 * @throws ArrayIndexOutOfBoundsException if <code>from</code> or
	 *                                        <code>to</code> are greater than
	 *                                        <code>arrayLength</code> or negative.
	 */
